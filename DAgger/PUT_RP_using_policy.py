...
class PUT_RP_using_policy():
    ...
    """
    beta is exploration rate = probability to select random action instead of following policy
    Returns strategy, num_nodes, bool whether timeout
    strategy is list of tuples ((G,W), e) of state and edge selected
    """
    def sample_strategy(self, profile, policy, true_winners, beta):
        ...
        while stackNode:
            ...
            if time.perf_counter() > END:
                print("TIMEOUT")
                return tuple(stats.strategy), stats, True, known_winners
            ...
            while len(E.edges()) != 0 or len(T) != 0:
                if true_winners is not None and known_winners == true_winners:
                    return tuple(stats.strategy), stats, False, known_winners
                ...
                for e in T:
                    if not G_tc.has_edge(e[1], e[0]):
                        ...
                        if random.random() < beta:
                            # With probability beta, randomly assign edge priority
                            prob = random.random()
                        else:
                ...
                new_items = list(np.random.choice(children, len(children), p=children_probs, replace=False))
                assert len(new_items) == len(children)

                # select with prob
                stackNode.extend(new_items)
                break
    ...
    """
    Returns num_nodes for full search
    No timeout, doesn't keep track of strategy
    Always selects edge with max prob
    """
    def test_policy(self, profile, policy):
        self.BEGIN = time.perf_counter()
